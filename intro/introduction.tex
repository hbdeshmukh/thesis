...
\todo{Add an executive summary for the thesis}
...
To get an idea of the problem of scheduling in database systems, we compare traditional database systems with MapReduce~\cite{mapreduce} based systems such as Hive~\cite{thusoo2010hive} in the next section.
...
Given the variety in terms of relational operators and their physical implementations, databases did not have a common scheduling abstraction for tasks within a query. 
The lack of a common scheduling abstraction could be a reason why schedulers are not considered a first class citizen in database architecture.

By having a scheduling abstraction the database system can have a greater control over how resources are managed, and allotted to its users.in
