...
We present the design and implementation of the Quickstep database system. 
Quickstep targets high speed in-memory relational analytics. 
...
Through benchmarking against other systems, we show that Quickstep is faster than other systems.
...
We describe the \textbf{nuts and bolts} of the scheduler.
We propose a novel task abstraction called \textit{work orders} used for query processing in Quickstep.
The abstraction is used by all the relational operators implemented in Quickstep.
The work orders abstraction gives powerful control to the Quickstep scheduler.
